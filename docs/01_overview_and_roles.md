...
DockerでWebアプリを動かすときに登場する「ユーザー」と「権限」を整理し、次の点を理解することを目的とします。

- アプリの利用者とLinuxユーザーは別物である
- Dockerコンテナ内では、アプリを非rootユーザーで動かす
- 一般利用者・管理者などの権限は、アプリ側で管理する
- ホストとコンテナでファイルを共有するときは、UID/GIDに注意する
- 誰にDockerの操作権限を与えるかを慎重に決める

この資料では、DjangoとPostgreSQLをDocker Composeで動かす構成を例にします。

---

## 2. 最初に理解するべき「3種類のユーザー」

Dockerを使用する構成では、少なくとも次の3種類のユーザーが登場します。
